...
int main(int argc, char *argv[])
{
    ...
    #pragma omp target enter data map(alloc: mpi_grid[0:(GX * mpi_GY)], mpi_grid_new[0:(GX * mpi_GY)])
    ...
    {
        #pragma omp target update to(mpi_grid[0:(GX * mpi_GY)], mpi_grid_new[0:(GX * mpi_GY)])

        #pragma omp target teams distribute parallel for
        for (int i = HY; i < mpi_GY - HY; i++)
        {
            for (int j = HX; j < GX - HX; j++)
            {
                mpi_grid_new[i * GX + j] = (mpi_grid[i * GX + j] + mpi_grid[(i - 1) * GX + j] + mpi_grid[(i + 1) * GX + j] + mpi_grid[i * GX + j - 1] + mpi_grid[i * GX + j + 1]) / 5;
            }
        }

        #pragma omp target update from(mpi_grid[0:(GX * mpi_GY)], mpi_grid_new[0:(GX * mpi_GY)])
        ...
        if (mpi_rank % 2 == 0)
        {
            if (mpi_rank > MPIROOT)
            {
                MPI_Send(mpi_grid_new + GX * HY, GX * HY, MPI_DOUBLE, mpi_rank - 1, mpi_rank, MPI_COMM_WORLD);
            }

            MPI_Send(mpi_grid_new + GX * mpi_GLY, GX * HY, MPI_DOUBLE, mpi_rank + 1, mpi_rank, MPI_COMM_WORLD);

            if (mpi_rank > MPIROOT)
            {
                MPI_Recv(mpi_grid_new, GX * HY, MPI_DOUBLE, mpi_rank - 1, mpi_rank - 1, MPI_COMM_WORLD, &mpi_status);
            }

            MPI_Recv(mpi_grid_new + GX * (HY + mpi_GLY), GX * HY, MPI_DOUBLE, mpi_rank + 1, mpi_rank + 1, MPI_COMM_WORLD, &mpi_status);
        }
        else
        {
    ...
    #pragma omp target exit data map(release: mpi_grid[0:(GX * mpi_GY)], mpi_grid_new[0:(GX * mpi_GY)])
