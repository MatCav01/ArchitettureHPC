...
int main(int argc, char *argv[])
{
    ...
    #pragma omp target enter data map(alloc: mpi_X[0:mpi_L], mpi_Y[0:mpi_L])
    ...
    #pragma omp target update to(mpi_X[0:mpi_L], mpi_Y[0:mpi_L])
    ...
    #pragma omp target update from(mpi_Y[0:mpi_L])

    MPI_Gather(mpi_Y, mpi_L, MPI_FLOAT, Y, mpi_L, MPI_FLOAT, 0, MPI_COMM_WORLD);
    
    dt = omp_get_wtime() - t0;

    #pragma omp target exit data map(release: mpi_X[0:mpi_L], mpi_Y[0:mpi_L])
